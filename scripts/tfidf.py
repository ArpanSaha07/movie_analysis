...
def compute_top_words_and_tfidf(input_file, output_file):
    ...
    # Create a DataFrame to store the top 10 words and TF-IDF scores for each category
    ...
    for category in coded_topics.unique():
        ...
        if category_tfidf_matrix.shape[0] != 0:
            ...
            top_tfidf_scores = avg_tfidf_scores[top_word_indices]

            # Add the top words and TF-IDF scores to the DataFrame with the category as the column name
            top_words_df[f'{category}_words'] = top_words
            top_words_df[f'{category}_tfidf'] = top_tfidf_scores

    # Save the top words and TF-IDF scores DataFrame to a CSV file
    top_words_df.to_csv(output_file, index=False)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute top words and TF-IDF scores using TF-IDF.")
    ...
    compute_top_words_and_tfidf(args.input, args.output)
